...
EOF"""
    ...
    dir ('packages') {
        ...
        sh "for i in *.zip; do bash $HOME/github-release.sh github_api_token=${github_token} owner=bacgroup repo=man_ads_java_less_client tag=man_ads_java_less_client_${BUILD_NUMBER}_${STAGE} filename=./\$i; done"
        sh "for i in *.deb; do bash $HOME/github-release.sh github_api_token=${github_token} owner=bacgroup repo=man_ads_java_less_client tag=man_ads_java_less_client_${BUILD_NUMBER}_${STAGE} filename=./\$i; done"
